  ...
  >
    ...
    <div className="space-y-3 md:space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-4">
        <div>
          <h3 className="text-xl md:text-2xl font-bold text-primary mb-1">{title}</h3>
          <p className="text-secondary flex items-center gap-1.5 text-xs md:text-sm">
            <MapPin size={14} /> {location}
          </p>
        </div>
        <span className="text-lg md:text-xl font-bold text-primary whitespace-nowrap">₦{price.toLocaleString()}</span>
      </div>
      <div className="flex flex-wrap gap-4 md:gap-6 text-on-surface-variant border-t border-outline-variant/20 pt-4 text-xs md:text-sm font-medium">
